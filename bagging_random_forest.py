# -------------------------------------------------------------------------
# AUTHOR: Abdur Rahman
# FILENAME: bagging_random_forrest.py
# SPECIFICATION: Compares a base, ensemble, and random forest classifier
...
for k in range(20):  # we will create 20 bootstrap samples here (k = 20). One classifier will be created for each bootstrap sample
    ...
    for i, testSample in enumerate(dbTest):
        ...
        if k == 0:  # for only the first base classifier, compare the prediction with the true label of the test sample here to start calculating its accuracy
            # --> add your Python code here
            if prediction == int(testSample[-1]):
                correct += 1
            total += 1
...
accuracy = 0.0
for i, testSample in enumerate(dbTest):
    forestPrediction = int(clf.predict([testSample[:-1]])[0])
    if forestPrediction == int(testSample[-1]):
        correct+=1
    total+=1
accuracy = correct/total
